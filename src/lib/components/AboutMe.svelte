<script>
  ...
  let dateLastPost, descLastPost;
  // get last post
  async function getLastPost() {
    const response = await fetch(
      "http://localhost:8800/api/posts/last/" + userId,
      {
        method: "GET",
      }
    );
    ...
    lastPost = data;
    if (lastPost) {
      dateLastPost = new Date(lastPost.updatedAt);
    ...
    <div class="bg-white p-3 rounded-md">
      {#if lastPost}
        <div class="font-light text-xs mb-3">
          {dateLastPost
            ? dateLastPost.toLocaleDateString("id", {
                day: "numeric",
                month: "long",
                year: "numeric",
              })
      ...
      {:else}
        ...
        <div class="text-sm">Recent posts you share will be displayed here</div>
      {/if}
    </div>
