<script>
  ...
  let visibility, desc, src, files, maxImage;
  ...
  async function addAchievement() {
    if (!maxImage) {
      ...
      const response = await fetch(
        "http://103.187.223.15:8800/api/achievements/",
        {
          method: "POST",
          body: dataUpdate,
        }
...
<section class="flex">
  <div class="py-8 px-4 w-1/2 lg:py-8">
    ...
    <form
      action="#"
      on:submit|preventDefault={addAchievement}
      ...
      <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
        <div>
          ...
          <input
            ...
            accept=".jpg, .jpeg, .png"
          />
          <div class="text-sm mt-2">*Maximum image size is 2MB</div>
        </div>
